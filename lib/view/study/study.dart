...
void showReviewCountPicker(BuildContext context) {
  // 这里可以复用已有的 BottomPicker 逻辑
  // 简单示例：
  Get.bottomSheet(
    Container(
      color: Colors.white,
      ...
      child: Column(
        ...
        children: [10, 20, 50, 100].map((count) => ListTile(
          title: Text("$count 个"),
          onTap: () {
            controller.reviewDailyCount.value = count;
            Get.back();
          },
        )).toList(),
      ),
...
Widget _buildOptionTile(String title, String value) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        decoration: BoxDecoration(
          color: Theme.of(context).scaffoldBackgroundColor, // 跟随主题
          boxShadow: [BoxShadow(blurRadius: 1, color: Colors.grey.withOpacity(0.1))]
        ),
        child: SafeArea(
          child: Container(
            height: 60,
   ...
   Widget buildEndView(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children:[
          Icon(Icons.verified_rounded, size: 80, color: isDark ? const Color(0xFF818CF8) : const Color(0xFF6366F1)),
          const SizedBox(height: 24),
          Text(
            mode == 'review' ? "复习任务已完成" : "本书已经学完",
            style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87),
          ),
          const SizedBox(height: 12),
          Text(
            mode == 'review' 
                ? "干得漂亮！建议休息一下，或者继续下一轮复习。" 
                : "恭喜您！这本书的所有单词都已学习完毕。\n您可以选择一种顺序重新学习本词书：",
          ...
          if (mode == 'new') ...[
            _buildRestartBtn("正序重学 (A-Z)", 'ASC', isDark),
            const SizedBox(height: 12),
            _buildRestartBtn("倒序重学 (Z-A)", 'DESC', isDark),
            const SizedBox(height: 12),
            _buildRestartBtn("乱序重学 (随机)", 'RANDOM', isDark),
          ] else ...[
            _buildRestartBtn("再复习今天", 'REVIEW_AGAIN', isDark),
            const SizedBox(height: 12),
            _buildRestartBtn("复习下一轮", 'REVIEW_NEXT', isDark),
          ],
          const SizedBox(height: 30),
          TextButton(
            onPressed: () => Get.offAllNamed("/main"),
            child: Text("返回首页", style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 16)),
          )
        ],
      ),
    );
  ...
  // 结算面板的按钮组件
  Widget _buildRestartBtn(String text, String action, bool isDark) {
    return SizedBox(
      width: double.infinity,
      height: 50,
      child: ElevatedButton(
        ...
        onPressed: () {
          if (action == 'REVIEW_AGAIN') {
            controller.reviewTodayAgain();
          } else if (action == 'REVIEW_NEXT') {
            controller.reloadStudy(); // 触发下一轮
          } else {
            controller.restartBook(action); // 触发不同顺序的重学
          }
        },
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      children: [
                        ...
                        InkWell(
                        onTap: () {
                            // 弹出类似 showWordPicker 的 BottomPicker 选择数字 (10, 20, 50, 100...)
                            showReviewCountPicker(context); 
                        },
                        child: _buildOptionTile("每日复习量", "${controller.reviewDailyCount.value}"),
                        ),
